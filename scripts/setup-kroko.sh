...
# Venv PRIMEIRO: o desempacotador roda nele (não há python3 no PATH em
# máquina limpa com pythons gerenciados pelo uv — falha real do gate v1).
...
if [ -f "$DESTINO/encoder.onnx" ]; then
  ...
  .venv-teste-kroko/bin/python scripts/kroko-desempacotar.py \
    "$DATA_DIR/$ARQUIVO" "$DESTINO"
fi
