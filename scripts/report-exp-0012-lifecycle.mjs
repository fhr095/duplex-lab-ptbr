...
import {
  ...
  evaluateExp0012
} from "./lib/exp-0012-analysis.mjs";
...
const DEFAULTS = Object.freeze({
  candidate: "eval/reports/exp-0012-browser-current.json",
  out: "eval/reports/exp-0012-output-interruption-lifecycle-v1.json",
...
const contractAudit = auditOutputInterruptionContract();
const report = evaluateExp0012({
...
console.log(
  `EXP-0012: ${report.decision.toUpperCase()} · ` +
    `replay ${report.gates.exactBrowserReplay ? "exato" : "divergente"} · ` +
    `render STOP ${report.metrics.browserRenderStopMs ?? "—"} ms · ` +
    `físico ${report.observations.physical.classification}`
);
