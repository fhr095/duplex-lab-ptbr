...
import {
  ...
  EXP0022_ORDER,
  EXP0022_PAYLOADS,
  EXP0022_WORKER_ENVELOPE_SCHEMA as ANALYSIS_WORKER_ENVELOPE_SCHEMA
...
import {
  ...
  captureExp0021ResponseBody as captureExp0022ResponseBody
} from "./lib/exp-0021-cdp-capture.mjs";

export const EXP0022_WORKER_ENVELOPE_SCHEMA =
  ANALYSIS_WORKER_ENVELOPE_SCHEMA;
export const EXP0022_TARGET_URL = EXP0022_CONFIG.targetUrl;
export const EXP0022_TTS_URL = EXP0022_CONFIG.ttsUrl;
export const EXP0022_WORKER_PAYLOADS = EXP0022_PAYLOADS;
export const EXP0022_WORKER_ORDER = EXP0022_ORDER;
...
function invariant(condition, message) {
  if (!condition) throw new Error(`EXP-0022 worker: ${message}`);
}
...
export function isExp0022AllowedNetworkUrl(
  value,
  targetUrl = EXP0022_TARGET_URL
) {
  let candidate;
  let target;
  try {
...
export function isExp0022TtsUrl(
  value,
  targetUrl = EXP0022_TARGET_URL
) {
  try {
...
export function isExp0022HealthUrl(
  value,
  targetUrl = EXP0022_TARGET_URL
) {
  try {
    const candidate = new URL(value);
    const expected = new URL("/api/health", targetUrl);
...
export function parseExp0022TtsPostData(value) {
  try {
...
export function exp0022BrowserAuditSource() {
  return `/*EXP0022_INSTALL_NEGATIVE_BUDGET_AUDIT*/ (() => {
    const audit = {
    ...
    Object.defineProperty(globalThis, "__exp0022Audit", {
      configurable: false,
      enumerable: false,
      writable: false,
      value: audit
...
export function exp0022ReadyExpression() {
  return `/*EXP0022_WAIT_READY*/ (() => Boolean(
    globalThis.__exp0022Audit &&
...
export function exp0022BrowserHealthExpression(navigationIndex) {
  invariant(
    Number.isSafeInteger(navigationIndex) &&
      navigationIndex >= 1 &&
      navigationIndex <= EXP0022_CONFIG.navigationCount,
    "índice da sonda health inválido"
  );
  const probeId = EXP0022_CONFIG.healthBinding
    .auditProbeIds[navigationIndex - 1];
  return `/*EXP0022_BROWSER_HEALTH_BINDING*/ (async () => {
    const probeId = ${JSON.stringify(probeId)};
    const url = new URL("/api/health", location.origin);
    url.searchParams.set(
      ${JSON.stringify(EXP0022_CONFIG.healthBinding.auditQueryName)},
      probeId
    );
    const response = await fetch(url, {
      cache: "no-store",
      headers: {
        ${JSON.stringify(EXP0022_CONFIG.healthBinding.auditHeaderName)}:
          ${JSON.stringify(EXP0022_CONFIG.healthBinding.auditHeaderValue)}
      }
    });
    if (!response.ok) {
      throw new Error("EXP0022 browser health HTTP " + response.status);
    }
    return {
      probeId,
      url: response.url,
...
export function exp0022BrowserFetchExpression(text) {
  invariant(nonEmptyText(text), "texto TTS do browser ausente");
  return `/*EXP0022_BROWSER_TTS_FETCH*/ (async () => {
...
function newHealthRecord(requestId) {
  return {
    requestId,
    requestWillBeSentCount: 0,
    responseReceivedCount: 0,
...
export function createExp0022NetworkTracker(options = {}) {
  const send = options.send;
  const targetUrl = options.targetUrl ?? EXP0022_TARGET_URL;
  const capture = options.capture ?? captureExp0022ResponseBody;
  ...
  function recordNetwork(
    ...
    const record = {
      ...
      postData: details.postData ?? null,
      tracksLoadingLifecycle: details.tracksLoadingLifecycle === true,
      responseReceivedCount: 0,
      responseUrl: null,
      status: null,
      mimeType: null,
    ...
    if (!isExp0022AllowedNetworkUrl(url, targetUrl)) {
      diagnostics.networkViolations.push({ requestId, url, method, type });
    }
    return record;
  }

  function latestNetworkRequest(requestId) {
    for (let index = networkRequests.length - 1; index >= 0; index -= 1) {
      if (networkRequests[index]?.requestId === requestId) {
        return networkRequests[index];
      }
    }
    return null;
  }
  ...
  function healthRecord(requestId) {
    if (!healthRecords.has(requestId)) {
      healthRecords.set(requestId, newHealthRecord(requestId));
    }
    return healthRecords.get(requestId);
  }
  ...
  function handleEvent(message) {
    ...
    if (message?.method === "Network.requestWillBeSent") {
      ...
      const auditProbeHeader = requestHeader(
        request.headers,
      ...
      if (isExp0022HealthUrl(request.url, targetUrl)) {
        const record = healthRecord(params.requestId);
        record.requestWillBeSentCount += 1;
        record.url = request.url ?? null;
        record.method = request.method ?? null;
        record.auditProbeHeader = auditProbeHeader;
        record.resourceType = params.type ?? null;
        record.loaderId = params.loaderId ?? null;
        record.frameId = params.frameId ?? null;
        record.redirectCount += params.redirectResponse !== undefined ? 1 : 0;
        record.requestTimestamp = params.timestamp ?? null;
        record.requestOrdinal = eventOrdinal;
      }
      if (!isExp0022TtsUrl(request.url, targetUrl)) return;
    ...
    if (message?.method === "Network.responseReceived") {
      ...
      const rawRecord = latestNetworkRequest(params.requestId);
      if (rawRecord === null || rawRecord.tracksLoadingLifecycle !== true) {
        recordStructural("NETWORK_RESPONSE_WITHOUT_REQUEST", {
          requestId: params.requestId
        });
      } else {
        rawRecord.responseReceivedCount += 1;
        rawRecord.responseUrl = response.url ?? null;
        rawRecord.status = response.status ?? null;
        rawRecord.mimeType = normalizeMimeType(response.mimeType);
        rawRecord.responseTimestamp = params.timestamp ?? null;
        rawRecord.responseOrdinal = eventOrdinal;
      }
      ...
      if (isExp0022HealthUrl(response.url, targetUrl)) {
        const record = healthRecord(params.requestId);
        record.responseReceivedCount += 1;
        record.url ??= response.url ?? null;
        record.status = response.status ?? null;
        record.mimeType = normalizeMimeType(response.mimeType);
        record.responseTimestamp = params.timestamp ?? null;
        record.responseOrdinal = eventOrdinal;
        if (record.requestWillBeSentCount === 0) {
          recordStructural("HEALTH_RESPONSE_WITHOUT_REQUEST", {
            requestId: params.requestId
          });
        }
      }
      if (!isExp0022TtsUrl(response.url, targetUrl)) return;
      const record = ttsRecord(params.requestId);
      record.responseReceivedCount += 1;
      record.url ??= response.url ?? null;
      record.status = response.status ?? null;
    ...
    if (message?.method === "Network.loadingFinished") {
      ...
      const rawRecord = latestNetworkRequest(params.requestId);
      if (rawRecord === null || rawRecord.tracksLoadingLifecycle !== true) {
        recordStructural("NETWORK_FINISH_WITHOUT_REQUEST", {
          requestId: params.requestId
        });
      } else {
        rawRecord.loadingFinishedCount += 1;
        rawRecord.encodedDataLength = params.encodedDataLength ?? null;
        rawRecord.finishedTimestamp = params.timestamp ?? null;
        rawRecord.finishedOrdinal = eventOrdinal;
      }
      if (healthRecords.has(params.requestId)) {
        const health = healthRecords.get(params.requestId);
        health.loadingFinishedCount += 1;
        health.finishedTimestamp = params.timestamp ?? null;
        health.finishedOrdinal = eventOrdinal;
        if (health.responseReceivedCount === 0) {
          recordStructural("HEALTH_FINISH_WITHOUT_RESPONSE", {
            requestId: params.requestId
          });
        }
      }
        ...
        : record.mimeType !== "audio/wav"
          ...
          : !Number.isFinite(record.encodedDataLength) ||
            ...
            : record.encodedDataLength >=
                EXP0022_NETWORK_ENABLE_OPTIONS.maxResourceBufferSize
    ...
    if (message?.method === "Network.loadingFailed") {
      ...
      const rawRecord = latestNetworkRequest(params.requestId);
      if (rawRecord === null || rawRecord.tracksLoadingLifecycle !== true) {
        recordStructural("NETWORK_FAILURE_WITHOUT_REQUEST", {
          requestId: params.requestId
        });
      } else {
        rawRecord.loadingFailedCount += 1;
        rawRecord.failedTimestamp = params.timestamp ?? null;
        rawRecord.failedOrdinal = eventOrdinal;
      }
      if (healthRecords.has(params.requestId)) {
        healthRecords.get(params.requestId).loadingFailedCount += 1;
        recordStructural("HEALTH_LOADING_FAILED", {
          requestId: params.requestId,
  ...
  async function finishUnit(browser, timeoutMs = UNIT_CAPTURE_TIMEOUT_MS) {
    ...
    const parsedPostData = parseExp0022TtsPostData(record.postData);
  ...
  function networkSnapshot(requestOffset = 0) {
    invariant(
      Number.isSafeInteger(requestOffset) && requestOffset >= 0 &&
        requestOffset <= networkRequests.length,
      "offset de snapshot da rede inválido"
    );
    const scoped = networkRequests.slice(requestOffset);
    const networkRequestIds = [...new Set(scoped.map((request) =>
      request?.requestId).filter(nonEmptyText))];
    const healthRequestIds = scoped.filter((request) =>
      isExp0022HealthUrl(request?.url, targetUrl))
      .map((request) => request.requestId);
    return Object.freeze({
      boundaryOrdinal: eventOrdinal,
      networkRequestIds,
      healthRequestIds,
      pendingRequestIds: networkRequestIds.filter((requestId) =>
        pendingNetworkRequestIds.has(requestId))
    });
  }
  ...
  return Object.freeze({
    ...
    getHealthRecord: (requestId) => clone(
      healthRecords.get(requestId) ?? newHealthRecord(requestId ?? null)
    ),
...
export function exp0022CdpUrlFromRoute(route, environment = {}) {
  ...
  const expected = canonicalCdpEndpoint(
    `${EXP0022_CONFIG.cdp.protocol}//${gateway}:${EXP0022_CONFIG.cdp.port}/`,
    gateway
  );
...
export function discoverExp0022CdpUrl(environment = process.env) {
  const route = execFileSync("ip", ["route", "show", "default"], {
    encoding: "utf8"
  });
  return exp0022CdpUrlFromRoute(route, environment);
}

export function exp0022BoundWebSocketUrl(cdpUrl, page) {
  ...
  const expectedWebSocketPath =
    `${EXP0022_CONFIG.cdp.webSocketPathPrefix}${page.id}`;
  invariant(
  ...
  return Object.freeze({
    ...
    binding: Object.freeze({
      endpoint,
      hostPolicy: EXP0022_CONFIG.cdp.hostPolicy,
      initialTarget: EXP0022_CONFIG.cdp.initialTarget,
      targetId: page.id,
...
export async function connectExp0022Chrome(options = {}) {
  ...
  const createUrl = new URL(
    `/json/new?${encodeURIComponent(EXP0022_CONFIG.cdp.initialTarget)}`,
    endpoint
  );
  ...
  const socketBinding = exp0022BoundWebSocketUrl(endpoint, page);
...
async function fetchHealth(targetUrl, fetchImpl = globalThis.fetch) {
  ...
  return normalizeHealthObservation(await response.json());
}
...
function normalizeHealthObservation(value) {
  ...
  return {
    ...
    vadControl: { engine: value?.vadControl?.engine ?? null },
...
function normalizeBrowserHealth(value) {
  ...
  return {
    ...
    status: value.status ?? null,
    mimeType: normalizeMimeType(value.mimeType),
    health: normalizeHealthObservation(value.health)
  };
}
...
function snapshotSummary(snapshot) {
  ...
  return {
    state: {
      active: snapshot?.state?.active ?? null,
      inputMode: snapshot?.state?.inputMode ?? null,
    ...
    audio: {
      ...
      transport: {
        socketReadyState:
          snapshot?.audio?.transport?.socketReadyState ?? null
      },
      vadControl: { state: snapshot?.audio?.vadControl?.state ?? null },
      vadShadow: {
        health: { state: snapshot?.audio?.vadShadow?.health?.state ?? null }
      },
      outputInterruptionLifecycle: {
        phase:
          snapshot?.audio?.outputInterruptionLifecycle?.phase ?? null
      }
    }
  };
}
...
export function createExp0022HealthBinding(input = {}) {
  const before = clone(input.before ?? {});
  const after = clone(input.after ?? {});
  const beforeNetworkIds = Array.isArray(before.networkRequestIds)
    ? before.networkRequestIds
    : [];
  const beforeHealthIds = Array.isArray(before.healthRequestIds)
    ? before.healthRequestIds
    : [];
  const afterNetworkIds = Array.isArray(after.networkRequestIds)
    ? after.networkRequestIds
    : [];
  const afterHealthIds = Array.isArray(after.healthRequestIds)
    ? after.healthRequestIds
    : [];
  const newNetworkRequestIds = afterNetworkIds.filter((requestId) =>
    !beforeNetworkIds.includes(requestId));
  const newHealthRequestIds = afterHealthIds.filter((requestId) =>
    !beforeHealthIds.includes(requestId));
  const bootstrapHealthRequestId = beforeHealthIds.length === 1
    ? beforeHealthIds[0]
    : null;
  const auditHealthRequestId = newHealthRequestIds.length === 1
    ? newHealthRequestIds[0]
    : null;
  const bootstrap = clone(input.getHealthRecord?.(
    bootstrapHealthRequestId
  ) ?? newHealthRecord(bootstrapHealthRequestId));
  const audit = clone(input.getHealthRecord?.(
    auditHealthRequestId
  ) ?? newHealthRecord(auditHealthRequestId));
  return Object.freeze({
    ...
    bootstrapFinishedBeforeAudit:
      bootstrapHealthRequestId !== null &&
      bootstrap.loadingFinishedCount === 1 &&
      Number.isFinite(bootstrap.finishedTimestamp) &&
      !(before.pendingRequestIds ?? []).includes(bootstrapHealthRequestId),
    bootstrap,
    audit
  });
}
...
function buildBudget(navigations, healthBefore, healthAfter) {
  ...
  const ttsRequests = allRequests.filter((request) =>
    isExp0022TtsUrl(request.url)).length;
  ...
  const externalRequests = allRequests.filter((request) =>
    !isExp0022AllowedNetworkUrl(request.url)).length;
  return {
    ttsRequests,
...
export async function runExp0022WorkerCampaign(options = {}) {
  const targetUrl = options.targetUrl ?? EXP0022_TARGET_URL;
  invariant(targetUrl === EXP0022_TARGET_URL, "target URL não pode variar");
  const fetchImpl = options.fetchImpl ?? globalThis.fetch;
  const connectChrome = options.connectChrome ?? connectExp0022Chrome;
  const startedAt = options.startedAt ??
    process.env.EXP0022_ATTEMPT_STARTED_AT ??
  ...
  try {
    await Promise.all([
      ...
      chrome.send("Network.enable", EXP0022_NETWORK_ENABLE_OPTIONS)
    ]);
    ...
    for (let navigationIndex = 1; navigationIndex <= 2; navigationIndex += 1) {
      ...
      const beforeAudit = tracker.networkSnapshot(requestsBefore);
      ...
      await tracker.waitForNetworkIdle();
      const afterAudit = tracker.networkSnapshot(requestsBefore);
        ...
        audit: {
          ...
          retryIssuedRequest:
            allRequests.slice(requestsBefore).filter((request) =>
              isExp0022TtsUrl(request.url)).length !== units.length,
