...
import { runExp0025BrowserCampaign } from
  "./lib/exp-0025-browser-harness.mjs";
...
export const EXP0025_WORKER_IPC_SCHEMA = "exp-0025-worker-ipc-v1";
export const EXP0025_SUPERVISOR_ACK_SCHEMA =
  "exp-0025-supervisor-ack-v1";
export const EXP0025_SUPERVISOR_START_SCHEMA =
  "exp-0025-supervisor-start-v1";
export const EXP0025_WORKER_COMMAND =
...
function invariant(condition, message) {
  if (!condition) throw new Error(`EXP-0025 worker: ${message}`);
}
...
export function validateExp0025WorkerMessage(message) {
  if (!exactKeys(message, [
    "kind", "payload", "schemaVersion", "sequence", "type"
  ]) || message.schemaVersion !== EXP0025_WORKER_IPC_SCHEMA ||
    message.kind !== "record" ||
...
export function validateExp0025SupervisorAck(ack, expectedSequence) {
  return exactKeys(ack, ["schemaVersion", "sequence", "status"]) &&
    ack.schemaVersion === EXP0025_SUPERVISOR_ACK_SCHEMA &&
    ack.sequence === expectedSequence && ack.status === "persisted";
}

export function validateExp0025SupervisorStart(message) {
  return exactKeys(message, ["schemaVersion", "startedAt", "status"]) &&
    message.schemaVersion === EXP0025_SUPERVISOR_START_SCHEMA &&
    message.status === "authorized" &&
    typeof message.startedAt === "string" &&
    Number.isFinite(Date.parse(message.startedAt));
}

export function waitForExp0025SupervisorStart(options = {}) {
  ...
  return new Promise((resolveStart, rejectStart) => {
    ...
    reader.once("line", (line) => {
      ...
      if (!validateExp0025SupervisorStart(message)) {
...
export function createExp0025IpcChannel(options = {}) {
  const input = options.input ?? process.stdin;
  ...
  reader.on("line", (line) => {
    ...
    if (!validateExp0025SupervisorAck(ack, pending.sequence)) {
...
export async function runExp0025Worker(options = {}) {
  const startedAt = options.startedAt ??
    process.env.EXP0025_WORKER_STARTED_AT ?? new Date().toISOString();
  const channel = options.channel ?? createExp0025IpcChannel(options.ipc);
  invariant(typeof channel?.emitRecord === "function",
    "channel.emitRecord ausente");
  return (options.runCampaign ?? runExp0025BrowserCampaign)({
    ...options.campaign,
    startedAt,
...
async function main() {
  ...
  const start = await waitForExp0025SupervisorStart();
  const channel = createExp0025IpcChannel();
  try {
    await runExp0025Worker({ channel, startedAt: start.startedAt });
  } catch (error) {
    try {
    ...
    process.stderr.write("EXP-0025 worker encerrou com falha\n");
    process.exitCode = 1;
  } finally {
    channel.close();
  }
