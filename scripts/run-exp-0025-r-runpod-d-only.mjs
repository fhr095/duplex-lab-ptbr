...
const POD_NAME_PREFIX = "duplex-exp0025-r-e-d-only-";
const INFRASTRUCTURE_ATTEMPT = 4;
const MAX_GPU_SECONDS = 2 * 60 * 60;
const MAX_COST_USD = 12;
const MAX_DOWNLOAD_BYTES = 70 * 1024 ** 3;
const PRIOR_CUMULATIVE_GPU_SECONDS = 1193.4900000095367;
const PRIOR_CUMULATIVE_COST_USD = 0.958107250007656;
...
const D_ONLY_PYTHON = "scripts/run_exp_0025_r_external_d_only.py";
...
function remoteScript({ allocationStart, hourlyUsd }) {
  const inputChecks = Object.entries(EXPECTED_STATIC_INPUT_SHA256)
    .map(([path, digest]) =>
...
async function main() {
  ...
  if (preflightOnly) {
    process.stdout.write(
      "[runpod-d-only] preflight local/provider passou; nenhum Pod criado\n"
    );
    return;
  }
  ...
  try {
    process.stdout.write("[runpod-d-only] criando o único Pod final H100...\n");
    ...
    remoteResult = await runRemote(
      connection,
      temporary,
      remoteScript({ allocationStart, hourlyUsd }),
