...
import {
  ...
  validateExp0025TrialCollection
} from "./exp-0025-browser-trial.mjs";
import {
...
export const EXP0025_TARGET_URL =
  EXP0025_CONFIG.targetUrl;
export const EXP0025_TTS_URL =
  new URL("/api/tts", EXP0025_TARGET_URL).href;
export const EXP0025_EXPECTED_WAV_SHA256 =
  EXP0025_CONFIG.expectedWavSha256;
export const EXP0025_EXPECTED_WAV_BYTES =
  EXP0025_CONFIG.expectedWavByteLength;
export const EXP0025_WORKER_ENVELOPE_SCHEMA =
  "exp-0025-physical-stop-worker-envelope-v1";
export const EXP0025_CAMPAIGN_DEADLINE_MS =
...
function invariant(condition, message) {
  if (!condition) {
    throw new Error(`EXP-0025 browser harness: ${message}`);
  }
}

invariant(
  EXP0020_FIXED_PHRASE === EXP0025_CONFIG.phrase &&
    EXP0020_NAVIGATION_COUNT === EXP0025_CONFIG.navigations &&
    EXP0020_TRIALS_PER_NAVIGATION === EXP0025_CONFIG.stopsPerNavigation &&
    EXP0020_TOTAL_TRIALS === EXP0025_CONFIG.totalStops &&
    EXP0020_TRIGGER_DELAY_MS ===
      EXP0025_CONFIG.triggerAfterRenderActiveMs &&
    EXP0020_TRIGGER_ERROR_MAX_MS ===
      EXP0025_CONFIG.triggerTimerErrorMaxMs &&
    EXP0020_POST_MARKER_HORIZON_MS ===
      EXP0025_CONFIG.postStopObservationMs &&
    JSON.stringify(EXP0021_NETWORK_ENABLE_OPTIONS) ===
      JSON.stringify(EXP0025_CONFIG.networkEnable),
  "contratos herdados EXP-0020/0021 divergem do freeze EXP-0025"
);
...
function assignedTtsRequest(tracker, requestOffset) {
  const raw = tracker.getNetworkRequests().slice(requestOffset).filter(
    (request) => isExp0022TtsUrl(request?.url, EXP0025_TARGET_URL)
  );
...
export function assertExp0025TargetUrl(value = EXP0025_TARGET_URL) {
  let href = null;
  try {
  ...
  invariant(href === EXP0025_TARGET_URL,
    `target precisa ser exatamente ${EXP0025_TARGET_URL}`);
  return href;
}

export async function fetchExp0025Health(
  targetUrl = EXP0025_TARGET_URL,
  fetchImpl = globalThis.fetch
) {
  assertExp0025TargetUrl(targetUrl);
...
export function validateExp0025PhysicalBinding(input = {}) {
  const errors = [];
  ...
  if (
    ...
    !isExp0022TtsUrl(request?.url, EXP0025_TARGET_URL)
...
export async function runExp0025BrowserCampaign(options = {}) {
  const startedAt = options.startedAt ?? new Date().toISOString();
  const targetUrl = assertExp0025TargetUrl(
    options.targetUrl ?? EXP0025_TARGET_URL
  );
  ...
  try {
    ...
    for (let navigationIndex = 1;
      ...
      for (let trialIndex = 1;
        ...
        const physical = normalizeExp0025BrowserTrialResult(
          await chrome.evaluate(
            exp0025TrialExpression({ navigationIndex, trialIndex }),
            TRIAL_TIMEOUT_MS
          ),
          identity
        );
        await persistRecord("PHYSICAL_TRIAL_RESULT", {
        ...
        const collectionValidation = validateExp0025TrialCollection(physical);
        trials.push({
          ...clone(physical),
          turnId: physical?.turnId ?? identity.trialId,
          timing: {
      ...
      const pageEvidence = await chrome.evaluate(
        "/*EXP0025_READ_NAVIGATION_EVIDENCE*/ (() => " +
          "structuredClone(globalThis.__duplexLab.snapshot()))()"
      );
      const navigation = {
        ...
        browser,
        trials,
        snapshot: clone(pageEvidence ?? null)
      };
  ...
  return Object.freeze({
    schemaVersion: EXP0025_WORKER_ENVELOPE_SCHEMA,
    status: "completed",
    startedAt,
...
export function validateExp0025WorkerEnvelopeShape(envelope) {
  const trials = envelope?.campaign?.navigations?.flatMap(
    (navigation) => navigation?.trials ?? []
  ) ?? [];
  const valid = envelope?.schemaVersion === EXP0025_WORKER_ENVELOPE_SCHEMA &&
    ...
    trials.every((trial) =>
      trial?.schemaVersion === EXP0025_BROWSER_TRIAL_RESULT_SCHEMA &&
      Object.values(EXP0025_BROWSER_TRIAL_STATUSES).includes(trial?.status)
    ) &&
    envelope.campaign?.browser?.product === EXP0025_CONFIG.chrome.product &&
    envelope.campaign?.browser?.protocolVersion ===
      EXP0025_CONFIG.chrome.protocolVersion;
  return Object.freeze({ valid, errors: valid ? [] : [
    "worker envelope EXP-0025 incompleto"
  ] });
}
