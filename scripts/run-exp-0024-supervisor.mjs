...
import {
  ...
  EXP0024_WORKER_COMMAND,
  validateExp0024WorkerMessage
} from "./run-exp-0024-worker.mjs";

export const EXP0024_RECEIPT_SCHEMA =
...
const SUPERVISOR_OWNED_TYPES = new Set([
  ...
  EXP0024_JOURNAL_FRAME_TYPES.workerOutcome
]);

function invariant(condition, message) {
  if (!condition) throw new Error(`EXP-0024 supervisor: ${message}`);
}
  ...
  for (const record of freeze.productionSources) {
    const [c0Bytes, baselineBytes] = await Promise.all([
      ...
      gitBytes(projectRoot, "show",
        `${EXP0024_EXP0019_EVIDENCE_COMMIT}:${record.path}`)
    ]);
    invariant(
...
function expectedStatusPaths(receiptPresent, journalPresent) {
  return [
    ...(journalPresent ? [EXP0024_BOUNDARY_PATHS.journal] : []),
    ...(receiptPresent ? [EXP0024_BOUNDARY_PATHS.receipt] : [])
  ].toSorted().map((path) => `?? ${path}`).join("\n");
}

export async function verifyExp0024CommittedBoundary(options = {}) {
  ...
  const openingPath = resolve(projectRoot, EXP0024_BOUNDARY_PATHS.opening);
  ...
  const freezeValidation = validateExp0024InstrumentationFreeze(freeze);
  const openingValidation = validateExp0024PhysicalStopAttempt(opening);
  ...
  invariant(validateExp0024OpeningCommitBoundary({
    freezeCommit,
    openingCommit,
  ...
  invariant(
    (await gitBytes(projectRoot, "show",
      `${freezeCommit}:${EXP0024_BOUNDARY_PATHS.freeze}`)).equals(freezeBytes),
    "freeze worktree/commit divergiu"
  );
  invariant(
    (await gitBytes(projectRoot, "show",
      `${openingCommit}:${EXP0024_BOUNDARY_PATHS.opening}`)).equals(openingBytes),
...
function receiptCore(boundary, input) {
  return {
    schemaVersion: EXP0024_RECEIPT_SCHEMA,
    ...
    workerCommand: EXP0024_WORKER_COMMAND,
    opening: {
      path: EXP0024_BOUNDARY_PATHS.opening,
...
export function createExp0024AttemptReceipt(boundary, input = {}) {
  const core = receiptCore(boundary, input);
  const receipt = Object.freeze({
    ...core,
    receiptSha256: `sha256:${canonicalSha256(core)}`
  });
  invariant(validateExp0024AttemptReceipt(receipt, boundary),
    "receipt criado é inválido");
  return receipt;
}

export function validateExp0024AttemptReceipt(receipt, boundary) {
  try {
...
export async function writeExp0024AtomicWriteOnce(path, bytes) {
...
export async function consumeExp0024Attempt(boundary, options = {}) {
  const consumedAt = options.consumedAt ?? new Date().toISOString();
  const receipt = createExp0024AttemptReceipt(boundary, {
    consumedAt,
  ...
  await (options.writeOnce ?? writeExp0024AtomicWriteOnce)(
    receiptPath,
    receiptBytes
  );
...
export async function createExp0024JournalWriter(options = {}) {
  const path = resolve(options.path);
  const firstFrame = createExp0024JournalFrame({
    ordinal: 1,
    type: EXP0024_JOURNAL_FRAME_TYPES.inProgress,
    payload: options.inProgress
  });
  const firstBytes = Buffer.from(serializeExp0024JournalFrame(firstFrame, {
    expectedOrdinal: 1
  }));
  await (options.writeOnce ?? writeExp0024AtomicWriteOnce)(path, firstBytes);
  ...
  async function append(type, payload) {
    ...
    const frame = createExp0024JournalFrame({
      ordinal: ordinal + 1,
      type,
      payload
    });
    const bytes = Buffer.from(serializeExp0024JournalFrame(frame, {
...
export async function runExp0024WorkerProcess(options = {}) {
  const projectRoot = resolve(options.projectRoot ?? PROJECT_ROOT);
  const append = options.append;
  invariant(typeof append === "function", "append do journal ausente");
  const child = (options.spawn ?? spawn)(
    process.execPath,
    [resolve(projectRoot, "scripts/run-exp-0024-worker.mjs")],
    {
      cwd: projectRoot,
      env: { ...process.env },
  ...
  async function processLine(line) {
    ...
    await writeStreamLine(child.stdin, {
      schemaVersion: EXP0024_SUPERVISOR_ACK_SCHEMA,
  ...
  await append(EXP0024_JOURNAL_FRAME_TYPES.workerStarted, {
    command: EXP0024_WORKER_COMMAND,
    pid: child.pid,
    startedAt
  });
  await writeStreamLine(child.stdin, {
    schemaVersion: EXP0024_SUPERVISOR_START_SCHEMA,
    startedAt,
    status: "authorized"
  });

  const deadlineMs = options.deadlineAtMs === undefined
    ? options.deadlineMs ?? EXP0024_CONFIG.attemptDeadlineMs
...
function recoveryStateFor(inspection, journalPresent) {
  ...
  if (inspection.status === EXP0024_JOURNAL_INSPECTION_STATES.truncatedTail) {
    return {
      executionState: EXP0024_EXECUTION_STATES.recoveryTruncatedTail,
      failureCode: "RECOVERY_TRUNCATED_JOURNAL_TAIL"
    };
  }
  return {
    executionState: EXP0024_EXECUTION_STATES.recoveryValidJournal,
    failureCode: "RECOVERY_SURVIVING_VALID_JOURNAL"
  };
}

export function createExp0024BoundarySummary(input = {}) {
  const worker = input.inspection.frames.find((frame) =>
    frame.type === EXP0024_JOURNAL_FRAME_TYPES.workerStarted);
  return {
    executionState: input.executionState,
    expectedRuntimeFingerprintSha256:
      input.boundary.expectedRuntimeFingerprintSha256,
    failureCode: input.failureCode,
    freezePath: EXP0024_BOUNDARY_PATHS.freeze,
    freezeVerified: true,
    ...
    journalFsyncBeforeAck: input.executionState ===
      EXP0024_EXECUTION_STATES.fresh,
    journalPath: EXP0024_BOUNDARY_PATHS.journal,
    journalSha256: input.inspection.sha256,
    journalVerified: input.inspection.status ===
      EXP0024_JOURNAL_INSPECTION_STATES.valid,
    openingPath: EXP0024_BOUNDARY_PATHS.opening,
    openingVerified: true,
    receiptConsumedAt: input.receipt.consumedAt,
    receiptPath: EXP0024_BOUNDARY_PATHS.receipt,
    receiptVerified: true,
    receiptWriteOnce: true,
    recoveryOnly: input.executionState !== EXP0024_EXECUTION_STATES.fresh,
    rerunAllowed: false,
...
export function createExp0024ReportFromArtifacts(input = {}) {
  const inspection = input.inspection ?? inspectExp0024Journal(
    input.journalBytes ?? Buffer.alloc(0)
  );
  const campaign = {
    boundary: createExp0024BoundarySummary({ ...input, inspection }),
    journal: inspection
  };
  return createExp0024Report({
...
export async function runExp0024Supervisor(options = {}) {
  const projectRoot = resolve(options.projectRoot ?? PROJECT_ROOT);
  const receiptPath = resolve(projectRoot, EXP0024_BOUNDARY_PATHS.receipt);
  const journalPath = resolve(projectRoot, EXP0024_BOUNDARY_PATHS.journal);
  const reportPath = resolve(projectRoot, EXP0024_BOUNDARY_PATHS.report);
  ...
  const boundary = await (options.verifyBoundary ??
    verifyExp0024CommittedBoundary)({
    projectRoot,
    receiptPresent,
    journalPresent
  });
  ...
  if (receiptPresent) {
    ...
    if (journalPresent) {
      ...
      inspection = inspectExp0024Journal(journalBytes);
      invariant([
        EXP0024_JOURNAL_INSPECTION_STATES.valid,
    ...
    ({ receipt, receiptBytes } = await (options.consumeAttempt ??
      consumeExp0024Attempt)(boundary));
    invariant(validateExp0024AttemptReceipt(receipt, boundary),
    ...
    try {
      try {
        workerResult = await (options.runWorker ?? runExp0024WorkerProcess)({
          projectRoot,
    ...
    journalBytes = await readFile(journalPath);
    inspection = inspectExp0024Journal(journalBytes);
    invariant(inspection.status === EXP0024_JOURNAL_INSPECTION_STATES.valid,
  ...
  await (options.writeReport ?? writeExp0024AtomicWriteOnce)(
    reportPath,
    reportBytes
  );
  return Object.freeze({
...
export async function verifyExp0024CliLock(projectRoot = PROJECT_ROOT) {
  const lockPath = resolve(projectRoot, EXP0024_BOUNDARY_PATHS.lock);
  ...
  invariant(parentCmdline.includes("flock") &&
    parentCmdline.includes(EXP0024_BOUNDARY_PATHS.lock),
...
export function parseExp0024SupervisorArgs(args) {
  invariant(
...
export function formatExp0024CheckSuccess(report) {
  invariant(Object.values(EXP0024_DECISIONS).includes(report?.decision),
    "decisão ausente no relatório");
  return `EXP-0024 CHECK PASS · decision=${report.decision} · ` +
    report.reportSha256;
}

export async function verifyExp0024RecordedEvidence(options = {}) {
  const projectRoot = resolve(options.projectRoot ?? PROJECT_ROOT);
  const boundary = await verifyExp0024CommittedBoundary({
    projectRoot,
  ...
  const inspection = inspectExp0024Journal(journalBytes);
  const expectedReport = createExp0024ReportFromArtifacts({
    boundary,
    receipt,
    receiptBytes,
    journalBytes,
  ...
  invariant(validateExp0024EvidenceCommitBoundary({
  ...
  for (const [path, bytes] of [
    [EXP0024_BOUNDARY_PATHS.receipt, receiptBytes],
    ...(journalPresent ? [[EXP0024_BOUNDARY_PATHS.journal, journalBytes]] : []),
    [EXP0024_BOUNDARY_PATHS.report, reportBytes]
  ]) {
    invariant(
...
async function main() {
  const args = parseExp0024SupervisorArgs(process.argv.slice(2));
  if (args.check) {
    const { report } = await verifyExp0024RecordedEvidence();
    console.log(formatExp0024CheckSuccess(report));
    return;
  }
  await verifyExp0024CliLock();
  const { report, state } = await runExp0024Supervisor();
  console.log(`EXP-0024 ${report.decision} (${state}) · ${report.reportSha256}`);
  console.log(`Report: ${EXP0024_BOUNDARY_PATHS.report}`);
}
