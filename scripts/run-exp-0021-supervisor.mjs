...
const RECEIPT_SCHEMA = "exp-0021-capture-attempt-consumption-v1";
const WORKER_TIMEOUT_MS = 5 * 60_000;
const HASH_PATTERN = /^sha256:[a-f0-9]{64}$/u;
const COMMIT_PATTERN = /^[a-f0-9]{40}$/u;

function invariant(condition, message) {
  if (!condition) throw new Error(`EXP-0021 supervisor: ${message}`);
}
...
export function createExp0021RuntimeFingerprintRecord(files) {
  invariant(
...
async function runtimeFingerprintAtCommit(projectRoot, commit) {
  ...
  return createExp0021RuntimeFingerprintRecord(files);
}
...
export async function verifyExp0021CommittedBoundary(options = {}) {
  const projectRoot = resolve(options.projectRoot ?? PROJECT_ROOT);
  const freezePath = resolve(projectRoot, EXP0021_BOUNDARY_PATHS.freeze);
  const attemptPath = resolve(projectRoot, EXP0021_BOUNDARY_PATHS.attempt);
  ...
  const freezeValidation = validateExp0021InstrumentationFreeze(freeze);
  const attemptValidation = validateExp0021CaptureAttempt(attempt);
  ...
  const attemptCommit = await gitText(
    ...
    "--format=%H",
    "--",
    EXP0021_BOUNDARY_PATHS.attempt
  );
  ...
  invariant(
    (await gitBytes(
      projectRoot,
      "show",
      `${attemptCommit}:${EXP0021_BOUNDARY_PATHS.attempt}`
    )).equals(attemptBytes),
  ...
  invariant(
    (await gitBytes(
      projectRoot,
      "show",
      `${freezeCommit}:${EXP0021_BOUNDARY_PATHS.freeze}`
    )).equals(freezeBytes),
  ...
  invariant(
    attempt.freeze.freezeCommit === freezeCommit &&
      ...
      attempt.freeze.path === EXP0021_BOUNDARY_PATHS.freeze &&
  ...
  invariant(
    attempt.campaign.command === EXP0021_OFFICIAL_COMMAND &&
      attempt.campaign.nonce === EXP0021_ATTEMPT_NONCE &&
  ...
  const [attemptAtFreeze, freezeAtC0] = await Promise.all([
    gitBytes(
      projectRoot,
      "show",
      `${freezeCommit}:${EXP0021_BOUNDARY_PATHS.attempt}`
    ).then(() => true, () => false),
    gitBytes(
      projectRoot,
      "show",
      `${freeze.runnerSourceCommit}:${EXP0021_BOUNDARY_PATHS.freeze}`
    ).then(() => true, () => false)
  ]);
  ...
  for (const record of freeze.productionSources) {
    ...
    invariant(
      sha256(c0Bytes) === record.fileSha256 &&
        sha256(baselineBytes) === record.exp0020FileSha256 &&
        c0Bytes.equals(baselineBytes),
      `${record.path} divergiu da baseline produtiva EXP-0020`
    );
...
function receiptCore(boundary, startedAt, processId) {
  return {
    ...
    attemptPath: EXP0021_BOUNDARY_PATHS.attempt,
...
export function createExp0021AttemptReceipt(input) {
...
export function validateExp0021AttemptReceipt(receipt, boundary) {
  try {
...
export async function consumeExp0021Attempt(boundary, options = {}) {
  ...
  invariant(
    validateExp0021AttemptReceipt(receipt, boundary),
    "recibo gerado é inválido"
  );
  const path = resolve(boundary.projectRoot, EXP0021_BOUNDARY_PATHS.receipt);
...
function campaignBoundaryRecord(boundary, receiptFileSha256) {
  return {
    freezePath: EXP0021_BOUNDARY_PATHS.freeze,
    attemptPath: EXP0021_BOUNDARY_PATHS.attempt,
    receiptPath: EXP0021_BOUNDARY_PATHS.receipt,
    freezeVerified: true,
...
function successfulAudits() {
  return Object.freeze(Object.fromEntries(
    EXP0021_AUDIT_KEYS.map((key) => [key, true])
  ));
}
...
export function validateExp0021WorkerEnvelopeShape(envelope) {
  return exactKeys(envelope, [
    ...
    envelope.campaign !== null && typeof envelope.campaign === "object";
}

export async function runExp0021WorkerProcess(options = {}) {
  const projectRoot = resolve(options.projectRoot ?? PROJECT_ROOT);
  const timeoutMs = options.timeoutMs ?? WORKER_TIMEOUT_MS;
  return new Promise((resolveWorker) => {
    const child = spawn(
      process.execPath,
      ["scripts/run-exp-0021-worker.mjs"],
      {
        cwd: projectRoot,
        env: {
          ...process.env,
          ...(options.startedAt
            ? { EXP0021_ATTEMPT_STARTED_AT: options.startedAt }
            : {})
        },
        stdio: ["ignore", "pipe", "pipe"]
      }
...
export function classifyExp0021WorkerResult(result, startedAt, completedAt) {
  ...
  if (!validateExp0021WorkerEnvelopeShape(envelope)) {
    return invalidWorkerEnvelope({
      startedAt,
      completedAt,
      code: "WORKER_MALFORMED_ENVELOPE",
...
export async function prepareExp0021SupervisorState(options = {}) {
  const projectRoot = resolve(options.projectRoot ?? PROJECT_ROOT);
  const receiptPath = resolve(projectRoot, EXP0021_BOUNDARY_PATHS.receipt);
  const reportPath = resolve(projectRoot, EXP0021_BOUNDARY_PATHS.report);
  ...
  if (receiptExists) {
    const boundary = await verifyExp0021CommittedBoundary({
      projectRoot,
...
export function validateExp0021RecordedBinding(input) {
  const errors = [];
  try {
    ...
    if (!validateExp0021AttemptReceipt(input.receipt, input.boundary)) {
    ...
    const reportValidation = validateExp0021Report(input.report);
...
async function writeCanonicalReport(projectRoot, report) {
  const path = resolve(projectRoot, EXP0021_BOUNDARY_PATHS.report);
  await writeAtomicWriteOnce(
    path,
...
export async function runExp0021Supervisor(options = {}) {
  const projectRoot = resolve(options.projectRoot ?? PROJECT_ROOT);
  const now = options.now ?? (() => new Date().toISOString());
  const state = options.preparedState ??
    await (options.prepareState ?? prepareExp0021SupervisorState)({
      projectRoot
    });
  ...
  if (state.mode === "fresh") {
    receiptRecord = await (options.consumeAttempt ?? consumeExp0021Attempt)(
      state.boundary,
      { startedAt: now() }
    );
  }
  invariant(
    receiptRecord && validateExp0021AttemptReceipt(
  ...
  if (state.mode === "orphan") {
    ...
    const runWorker = options.runWorker ?? runExp0021WorkerProcess;
    let workerResult;
    try {
    ...
    workerEnvelope = classifyExp0021WorkerResult(
      workerResult,
  ...
  const createReport = options.createReport ?? createExp0021Report;
  ...
  const validation = validateExp0021RecordedBinding({
...
export async function verifyExp0021RecordedEvidence(options = {}) {
  const projectRoot = resolve(options.projectRoot ?? PROJECT_ROOT);
  const boundary = await verifyExp0021CommittedBoundary({ projectRoot });
  const [receiptBytes, reportBytes] = await Promise.all([
    readFile(resolve(projectRoot, EXP0021_BOUNDARY_PATHS.receipt)),
    readFile(resolve(projectRoot, EXP0021_BOUNDARY_PATHS.report))
  ]);
  const receipt = JSON.parse(receiptBytes.toString("utf8"));
  ...
  if (options.requireCommitted === true) {
    ...
    invariant(
      COMMIT_PATTERN.test(receiptCommit) && receiptCommit === reportCommit,
      "receipt e report precisam nascer no mesmo commit"
    );
    invariant(
      await gitText(projectRoot, "rev-parse", `${reportCommit}^`) ===
        boundary.attemptCommit,
      "evidência precisa ser filha direta da abertura"
    );
    invariant(
      isDeepStrictEqual(
      ...
      "commit de evidência pode adicionar somente receipt e report"
    );
    const [committedReceipt, committedReport, headReceipt, headReport] =
      await Promise.all([
        gitBytes(
          projectRoot,
          "show",
          `${receiptCommit}:${EXP0021_BOUNDARY_PATHS.receipt}`
        ),
        gitBytes(
          projectRoot,
          "show",
          `${reportCommit}:${EXP0021_BOUNDARY_PATHS.report}`
        ),
        gitBytes(projectRoot, "show", `HEAD:${EXP0021_BOUNDARY_PATHS.receipt}`),
        gitBytes(projectRoot, "show", `HEAD:${EXP0021_BOUNDARY_PATHS.report}`)
      ]);
    invariant(
      committedReceipt.equals(receiptBytes) &&
        committedReport.equals(reportBytes) &&
        headReceipt.equals(receiptBytes) && headReport.equals(reportBytes),
      "blobs de evidência divergiram do commit ou HEAD"
    );
    await gitBytes(
      projectRoot,
      "merge-base",
...
export function parseExp0021SupervisorArgs(args) {
  invariant(
    args.length === 0 ||
...
async function main() {
  const options = parseExp0021SupervisorArgs(process.argv.slice(2));
  if (options.check) {
    const { report } = await verifyExp0021RecordedEvidence({
      requireCommitted: true
    });
    console.log(
      `EXP-0021 report PASS: ${report.decision} · ${report.reportSha256}`
    );
    return;
  }
  const { report, state } = await runExp0021Supervisor();
  console.log(
    `EXP-0021 ${report.decision} (${state}) · ${report.reportSha256}`
  );
  console.log(`Report: ${EXP0021_BOUNDARY_PATHS.report}`);
}
