...
import { EXP0021_CONFIG } from "./exp-0021-capture-qualification.mjs";

export const EXP0021_FREEZE_SCHEMA =
  "exp-0021-instrumentation-freeze-v1";
export const EXP0021_ATTEMPT_SCHEMA =
  "exp-0021-capture-attempt-v1";
export const EXP0021_ATTEMPT_NONCE = "exp-0021-official-v0.1";
export const EXP0021_OFFICIAL_COMMAND =
  "node scripts/run-exp-0021-supervisor.mjs";
export const EXP0021_EXP0020_EVIDENCE_COMMIT =
  "404a06d5a7eba90ae72690e8870966770b97902a";
export const EXP0021_RUNTIME_FINGERPRINT_ROOTS = Object.freeze([
  "src",
  "web",
  "package.json",
  "package-lock.json",
...
export const EXP0021_RUNTIME_FINGERPRINT_ALGORITHM =
  "sha256-source-tree-v1";

export const EXP0021_PREREGISTRATION_PATH =
  "docs/experiments/EXP-0021-cdp-capture-recovery.md";
export const EXP0021_EXP0020_REPORT_PATH =
  "eval/reports/exp-0020-stop-order-v0.1.json";
export const EXP0021_EXP0020_CLOSEOUT_PATH =
  "docs/experiments/EXP-0020-closeout.md";
...
export const EXP0021_PRODUCTION_SOURCE_PATHS = Object.freeze([
...
export const EXP0021_INSTRUMENTATION_SOURCE_PATHS = Object.freeze([
  ".gitignore",
  "package.json",
  "scripts/freeze-exp-0021-instrumentation.mjs",
  "scripts/lib/exp-0021-cdp-capture.mjs",
  "scripts/open-exp-0021-capture-attempt.mjs",
  "scripts/run-exp-0021-supervisor.mjs",
  "scripts/run-exp-0021-worker.mjs",
  "src/eval/exp-0021-boundary.mjs",
  "src/eval/exp-0021-capture-qualification.mjs",
  "tests/exp-0021-analysis.test.mjs",
  "tests/exp-0021-boundary.test.mjs",
  "tests/exp-0021-cdp-capture.test.mjs",
  "tests/exp-0021-supervisor.test.mjs",
  "tests/exp-0021-worker.test.mjs"
].toSorted());

export const EXP0021_TRIAL_ORDER = deepFreeze([
...
export const EXP0021_GIT_TOPOLOGY = deepFreeze({
  ...
  evidenceAllowedPaths: [EXP0021_RECEIPT_PATH, EXP0021_REPORT_PATH]
});

const HASH_PATTERN = /^sha256:[a-f0-9]{64}$/u;
const COMMIT_PATTERN = /^[a-f0-9]{40}$/u;
const TARGET_URL =
  "http://localhost:4173/?automation=1&experiment=0021";
  ...
  return Array.isArray(records) &&
    ...
    records.every((record) => {
      const keys = production
        ? ["exp0020FileSha256", "fileSha256", "path"]
        : ["fileSha256", "path"];
      return exactKeys(record, keys) && hash(record.fileSha256) &&
        (!production ||
          hash(record.exp0020FileSha256) &&
          record.fileSha256 === record.exp0020FileSha256);
    });
}

function freezeCore(input) {
  return {
    schemaVersion: EXP0021_FREEZE_SCHEMA,
    ...
    sourceBaseline: {
      ...
      evidenceCommit: EXP0021_EXP0020_EVIDENCE_COMMIT
    },
    runtimeBinding: structuredClone(input.runtimeBinding),
    artifacts: structuredClone(input.artifacts),
    config: structuredClone(EXP0021_CONFIG),
    configCanonicalSha256: `sha256:${canonicalSha256(EXP0021_CONFIG)}`,
    ...
    gitTopology: structuredClone(EXP0021_GIT_TOPOLOGY),
    boundary: {
...
export function createExp0021InstrumentationFreeze(input = {}) {
  ...
  if (!validation.valid) {
    throw new TypeError(
      `freeze EXP-0021 inválido: ${validation.errors.join("; ")}`
    );
  }
  return freeze;
}

export function validateExp0021InstrumentationFreeze(freeze) {
  const errors = [];
  try {
    if (!exactKeys(freeze, [
      "artifacts",
      ...
      freeze?.schemaVersion !== EXP0021_FREEZE_SCHEMA ||
      freeze?.experimentId !== "EXP-0021" ||
    ...
    if (!exactKeys(freeze?.artifacts, [
      ...
      !artifactValid(
        ...
        EXP0021_PREREGISTRATION_PATH
      ) ||
      !artifactValid(
        freeze?.artifacts?.exp0020Report,
        EXP0021_EXP0020_REPORT_PATH
      ) ||
      !artifactValid(
        freeze?.artifacts?.exp0020Closeout,
        EXP0021_EXP0020_CLOSEOUT_PATH
      )
    ...
    if (!sourceListValid(
      ...
      true
    )) errors.push("fontes de produção divergiram do evidence commit EXP-0020");
    if (!sourceListValid(
    ...
    if (!isDeepStrictEqual(freeze?.gitTopology, EXP0021_GIT_TOPOLOGY)) {
...
function attemptCampaign() {
  return {
    ...
    totalRequests: 4,
    trialOrder: structuredClone(EXP0021_TRIAL_ORDER),
    reportPath: EXP0021_REPORT_PATH,
...
function attemptCore(input) {
  return {
    schemaVersion: EXP0021_ATTEMPT_SCHEMA,
    experimentId: "EXP-0021",
...
export function createExp0021CaptureAttempt(input = {}) {
  const core = attemptCore(input);
  const attempt = deepFreeze({
    ...core,
    captureAttemptSha256: `sha256:${canonicalSha256(core)}`
  });
  const validation = validateExp0021CaptureAttempt(attempt);
  if (!validation.valid) {
    throw new TypeError(
      `tentativa EXP-0021 inválida: ${validation.errors.join("; ")}`
    );
  }
  return attempt;
}

export function validateExp0021CaptureAttempt(attempt) {
  const errors = [];
  try {
    if (!exactKeys(attempt, [
      ...
      attempt?.schemaVersion !== EXP0021_ATTEMPT_SCHEMA ||
      attempt?.experimentId !== "EXP-0021" ||
    ...
    if (!exactKeys(attempt?.freeze, [
      ...
      attempt?.freeze?.path !== EXP0021_FREEZE_PATH ||
    ...
    if (!isDeepStrictEqual(attempt?.gitTopology, EXP0021_GIT_TOPOLOGY)) {
...
export const EXP0021_BOUNDARY_PATHS = deepFreeze({
  preregistration: EXP0021_PREREGISTRATION_PATH,
  exp0020Report: EXP0021_EXP0020_REPORT_PATH,
  exp0020Closeout: EXP0021_EXP0020_CLOSEOUT_PATH,
