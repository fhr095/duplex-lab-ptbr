...
export const EXP0024_JOURNAL_SCHEMA = "exp-0024-journal-frame-v1";
export const EXP0024_JOURNAL_NONCE = "exp-0024-official-v0.1";

export const EXP0024_JOURNAL_FRAME_TYPES = deepFreeze({
  ...
  physicalTrialCompleted: "PHYSICAL_TRIAL_COMPLETED",
...
export const EXP0024_JOURNAL_INSPECTION_STATES = deepFreeze({
  empty: "EMPTY",
  valid: "VALID",
...
export const EXP0024_JOURNAL_FORBIDDEN_KEYS = deepFreeze([
  "body",
  "bodyBase64",
  "base64Body",
  "base64",
...
export const EXP0024_JOURNAL_PAYLOAD_KEYS = deepFreeze({
...
const FRAME_TYPE_SET = new Set(Object.values(EXP0024_JOURNAL_FRAME_TYPES));
const FORBIDDEN_KEY_SET = new Set(
  EXP0024_JOURNAL_FORBIDDEN_KEYS.map((key) => key.toLowerCase())
);
...
function validatePayload(type, payload, errors) {
  const keys = EXP0024_JOURNAL_PAYLOAD_KEYS[type];
  ...
  switch (type) {
    ...
    case EXP0024_JOURNAL_FRAME_TYPES.physicalTrialCompleted:
      ...
      add(errors, nullableString(payload.requestId),
        "PHYSICAL_TRIAL_COMPLETED.requestId inválido");
      add(errors, validDate(payload.completedAt),
        "PHYSICAL_TRIAL_COMPLETED.completedAt inválido");
      add(errors, plainObject(payload.trial) &&
        Object.keys(payload.trial).length > 0,
      "PHYSICAL_TRIAL_COMPLETED.trial inválido");
      break;
...
export function validateExp0024JournalFrame(frame, options = {}) {
  const errors = [];
  try {
    ...
    add(errors, frame.schemaVersion === EXP0024_JOURNAL_SCHEMA,
      "schemaVersion divergente");
    add(errors, frame.nonce === EXP0024_JOURNAL_NONCE,
      "nonce divergente");
...
export function createExp0024JournalFrame(input = {}) {
  const candidate = {
    schemaVersion: input.schemaVersion ?? EXP0024_JOURNAL_SCHEMA,
    nonce: input.nonce ?? EXP0024_JOURNAL_NONCE,
  ...
  if (!validation.valid) {
    throw new TypeError(
      `frame EXP-0024 inválido: ${validation.errors.join("; ")}`
    );
  }
...
function validateFrameSequence(frames) {
  ...
  if (frames[0]?.type !== EXP0024_JOURNAL_FRAME_TYPES.inProgress) {
    errors.push("primeiro frame precisa ser IN_PROGRESS");
  }
  for (let index = 1; index < frames.length; index += 1) {
    if (frames[index]?.type === EXP0024_JOURNAL_FRAME_TYPES.inProgress) {
  ...
  const outcomes = frames
    ...
    .filter(({ frame }) =>
      frame?.type === EXP0024_JOURNAL_FRAME_TYPES.workerOutcome);
...
export function serializeExp0024JournalFrame(frame, options = {}) {
  const validation = validateExp0024JournalFrame(frame, options);
  if (!validation.valid) {
    throw new TypeError(
      `frame EXP-0024 inválido: ${validation.errors.join("; ")}`
    );
...
export function serializeExp0024Journal(frames) {
  if (!Array.isArray(frames)) {
    throw new TypeError("journal EXP-0024 exige array de frames");
  }
  const normalized = frames.map((frame, index) => {
    const validation = validateExp0024JournalFrame(frame, {
  ...
  if (sequenceErrors.length > 0) {
    throw new TypeError(`sequência EXP-0024 inválida: ${sequenceErrors.join("; ")}`);
  }
...
function resultFor(bytes, tail, status, frames, errors) {
  return deepFreeze({
    status,
    valid: status === EXP0024_JOURNAL_INSPECTION_STATES.valid,
    sha256: sha256(bytes),
...
export function inspectExp0024Journal(bytes) {
  if (!(Buffer.isBuffer(bytes) || bytes instanceof Uint8Array)) {
    throw new TypeError("inspectExp0024Journal exige Buffer ou Uint8Array");
  }
  ...
  if (journal.byteLength === 0) {
    return resultFor(
      journal,
      empty,
      EXP0024_JOURNAL_INSPECTION_STATES.empty,
      [],
      []
    );
  }
