...
import {
  EXP0025_RECEIPT_SCHEMA,
  consumeExp0025Attempt,
  createExp0025AttemptReceipt,
  createExp0025JournalWriter,
  createExp0025ReportFromArtifacts,
  runExp0025Supervisor,
  runExp0025WorkerProcess,
  validateExp0025AttemptReceipt,
  writeExp0025AtomicWriteOnce
} from "../scripts/run-exp-0025-supervisor.mjs";
import {
...
import {
  runExp0025Preflight,
  validateExp0025PreflightHealth
} from "../scripts/open-exp-0025-causal-render-onset-attempt.mjs";

const NOW = "2026-08-03T12:00:00.000Z";
const COMMIT = "a".repeat(40);
const OPENING_HASH = `sha256:${"b".repeat(64)}`;

function fakeBoundary(projectRoot = "/tmp/exp0025-fixture") {
  const opening = {
    campaign: { nonce: "exp-0025-official-v0.1" },
...
function receipt(boundary, input = {}) {
  return createExp0025AttemptReceipt(boundary, {
...
async function temporaryRoot(t) {
  const root = await mkdtemp(join(tmpdir(), "exp0025-supervisor-"));
...
test("receipt write-once liga opening, nonce, PID e deadline", async (t) => {
  ...
  assert.equal(attempt.schemaVersion, EXP0025_RECEIPT_SCHEMA);
  assert.equal(validateExp0025AttemptReceipt(attempt, boundary), true);
  assert.equal(attempt.opening.commit, COMMIT);
  assert.equal(attempt.deadlineMs, 600_000);
  assert.equal(attempt.rerunAllowed, false);
  const consumed = await consumeExp0025Attempt(boundary, {
    consumedAt: NOW,
    supervisorPid: 123
  });
  assert.deepEqual(consumed.receipt, attempt);
  await assert.rejects(
    consumeExp0025Attempt(boundary, { consumedAt: NOW, supervisorPid: 123 }),
    /EEXIST/u
  );
});
...
test("preflight lê apenas health/CDP version e não abre target ou STOP", async () => {
  ...
  assert.equal(validateExp0025PreflightHealth(health, "c".repeat(64)), true);
  assert.equal(validateExp0025PreflightHealth({
    ...health,
    brain: "openai"
  }, "c".repeat(64)), false);
  const urls = [];
  const result = await runExp0025Preflight({
    freeze: {
...
test("journal nasce atomicamente com IN_PROGRESS e fsynca append", async (t) => {
  ...
  const boundary = fakeBoundary(root);
  const attempt = receipt(boundary);
  const path = join(root, EXP0025_BOUNDARY_PATHS.journal);
  const writer = await createExp0025JournalWriter({
    path,
    inProgress: inProgress(boundary, attempt)
  });
  await writer.append(EXP0025_JOURNAL_FRAME_TYPES.workerStarted, {
    command: "node scripts/run-exp-0025-worker.mjs",
    pid: 456,
    startedAt: NOW
  });
  await writer.close();
  const inspected = inspectExp0025Journal(await readFile(path));
  ...
  await assert.rejects(createExp0025JournalWriter({
    path,
    inProgress: inProgress(boundary, attempt)
  }), /EEXIST/u);
});
...
test("recovery sem journal produz invalidação canônica e não avalia físico", () => {
  ...
  const attempt = receipt(boundary);
  const inspection = inspectExp0025Journal(Buffer.alloc(0));
  ...
  assert.equal(report.decision, EXP0025_DECISIONS.invalidate);
...
test("supervisor recovery nunca chama worker e grava um único relatório", async (t) => {
  ...
  await writeProjectFile(root, EXP0025_BOUNDARY_PATHS.receipt, receiptBytes);
  let workerCalled = false;
  let reportWrites = 0;
  const result = await runExp0025Supervisor({
    projectRoot: root,
  ...
  assert.equal(result.state,
    EXP0025_EXECUTION_STATES.recoveryWithoutJournal);
  assert.equal(result.report.decision, EXP0025_DECISIONS.invalidate);
});
...
test("recovery recusa frame completo malformado sem escrever report", async (t) => {
  ...
  let reportWrites = 0;
  await assert.rejects(runExp0025Supervisor({
    projectRoot: root,
    verifyBoundary: async () => boundary,
...
test("execução fresca ordena receipt → journal → worker → outcome → report", async (t) => {
  ...
  const result = await runExp0025Supervisor({
    ...
    consumeAttempt: async () => {
      ...
      await writeProjectFile(root, EXP0025_BOUNDARY_PATHS.receipt, bytes);
  ...
  assert.equal(result.state, EXP0025_EXECUTION_STATES.fresh);
...
test("processo worker recebe start só após WORKER_STARTED e ACK só após append", async () => {
  ...
  child.stdin.on("data", (line) => {
    ...
    if (message.schemaVersion === EXP0025_SUPERVISOR_START_SCHEMA) {
      events.push("start");
      workerSequence += 1;
    ...
    } else if (message.schemaVersion === EXP0025_SUPERVISOR_ACK_SCHEMA) {
      events.push("ack");
      queueMicrotask(() => child.emit("close", 0, null));
    }
  });
  const result = await runExp0025WorkerProcess({
    projectRoot: "/tmp",
    spawn: () => child,
    startedAt: NOW,
    deadlineMs: 1_000,
...
test("atomic write-once nunca sobrescreve target existente", async (t) => {
  ...
  await writeExp0025AtomicWriteOnce(path, Buffer.from("primeiro\n"));
  await assert.rejects(
    writeExp0025AtomicWriteOnce(path, Buffer.from("segundo\n")),
    /EEXIST/u
  );
