...
import {
  ...
  createExp0024IpcChannel,
  runExp0024Worker,
  validateExp0024SupervisorAck,
  validateExp0024SupervisorStart,
  validateExp0024WorkerMessage,
  waitForExp0024SupervisorStart
...
test("IPC do worker possui schema fechado e reaproveita o validador do journal", () => {
  assert.equal(EXP0024_WORKER_COMMAND,
    "node scripts/run-exp-0024-worker.mjs");
  assert.equal(validateExp0024WorkerMessage(message()), true);
  assert.equal(validateExp0024WorkerMessage({ ...message(), extra: true }), false);
  assert.equal(validateExp0024WorkerMessage({
    ...message(),
    payload: { ...diagnosticPayload(), bytes: [1, 2] }
  }), false);
  assert.equal(validateExp0024SupervisorAck({
  ...
  assert.equal(validateExp0024SupervisorStart({
...
test("worker não inicia antes da autorização tipada do supervisor", async () => {
  ...
  const pending = waitForExp0024SupervisorStart({ input, timeoutMs: 1_000 });
...
test("canal de ACK assume o mesmo stdin depois da autorização", async () => {
  const input = new PassThrough();
  const startPending = waitForExp0024SupervisorStart({
    input,
    timeoutMs: 1_000
  });
  input.write(`${JSON.stringify({
  ...
  const channel = createExp0024IpcChannel({ input, output, timeoutMs: 1_000 });
...
test("runExp0024Worker injeta início congelado e canal sem monólito stdout", async () => {
  const emitted = [];
  const channel = {
    emitRecord: async (type, payload) => emitted.push({ type, payload })
  };
  const result = await runExp0024Worker({
    channel,
    startedAt: NOW,
