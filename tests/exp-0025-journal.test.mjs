...
import { createExp0025InstrumentFailure } from
  "../scripts/lib/exp-0025-browser-trial.mjs";

import {
  EXP0025_JOURNAL_FORBIDDEN_KEYS,
  EXP0025_JOURNAL_FRAME_TYPES,
  EXP0025_JOURNAL_INSPECTION_STATES,
  EXP0025_JOURNAL_NONCE,
  EXP0025_JOURNAL_PAYLOAD_KEYS,
  EXP0025_JOURNAL_SCHEMA,
  ...
  switch (type) {
    case EXP0025_JOURNAL_FRAME_TYPES.inProgress:
      return {
        ...
        opening: {
          ...
          commit: COMMIT,
          fileSha256: HASH,
          path: "eval/commitments/exp-0025-attempt-v0.1.json"
        },
        pid: 123,
        startedAt: NOW
      };
    ...
    case EXP0025_JOURNAL_FRAME_TYPES.physicalTrialResult:
      return {
        ...
        navigationIndex: 1,
        requestId: "request-1",
        trial: createExp0025InstrumentFailure({
          navigationIndex: 1,
          trialIndex: 1,
          code: "FIXTURE_FAILURE",
          message: "fixture tipada"
        }),
        trialId: "exp0020-nav-1-trial-1",
        trialIndex: 1,
        turnId: "exp0020-nav-1-trial-1"
      };
...
function allFrames() {
  return Object.values(EXP0025_JOURNAL_FRAME_TYPES)
    .map((type, index) => frame(type, index + 1));
}

test("expõe schemas fechados para todos os fatos autoritativos", () => {
  assert.equal(EXP0025_JOURNAL_SCHEMA, "exp-0025-journal-frame-v1");
  assert.equal(EXP0025_JOURNAL_NONCE, "exp-0025-official-v0.1");
  assert.deepEqual(Object.values(EXP0025_JOURNAL_FRAME_TYPES), [
    "IN_PROGRESS",
    "WORKER_STARTED",
    "HEALTH_BEFORE",
    "BROWSER_BOUND",
  ...
  assert.deepEqual(
    Object.keys(EXP0025_JOURNAL_PAYLOAD_KEYS).toSorted(),
    Object.values(EXP0025_JOURNAL_FRAME_TYPES).toSorted()
  );
  for (const key of [
    "body",
    "bodyBase64",
    ...
    "Uint8Array",
    "base64"
  ]) assert.ok(EXP0025_JOURNAL_FORBIDDEN_KEYS.includes(key));
});

test("cria e valida cada frame com nonce, ordinal e payload exatos", () => {
  for (const candidate of allFrames()) {
    assert.equal(candidate.schemaVersion, EXP0025_JOURNAL_SCHEMA);
    assert.equal(candidate.nonce, EXP0025_JOURNAL_NONCE);
    assert.equal(
      validateExp0025JournalFrame(candidate, {
    ...
    assert.deepEqual(
      ...
      [...EXP0025_JOURNAL_PAYLOAD_KEYS[candidate.type]].toSorted()
    );
  }
});
...
test("serializa NDJSON canônico e reconstrói apenas frames completos", () => {
  ...
  const inspected = inspectExp0025Journal(Buffer.from(serialized));
  assert.equal(inspected.status, EXP0025_JOURNAL_INSPECTION_STATES.valid);
...
test("EMPTY e prefixo IN_PROGRESS completo são estados distintos", () => {
  const empty = inspectExp0025Journal(Buffer.alloc(0));
  assert.equal(empty.status, EXP0025_JOURNAL_INSPECTION_STATES.empty);
  assert.equal(empty.valid, false);
  assert.deepEqual(empty.frames, []);
  assert.equal(empty.sha256, sha256(Buffer.alloc(0)));

  const onlyOpening = serializeExp0025Journal([frame(
    EXP0025_JOURNAL_FRAME_TYPES.inProgress,
    1
  )]);
  const validPrefix = inspectExp0025Journal(Buffer.from(onlyOpening));
  assert.equal(validPrefix.status, EXP0025_JOURNAL_INSPECTION_STATES.valid);
  assert.equal(validPrefix.completeFrameCount, 1);
});

test("TRUNCATED_TAIL preserva prefixo e hashes sem interpretar a tail", () => {
  const first = serializeExp0025Journal([frame(
    EXP0025_JOURNAL_FRAME_TYPES.inProgress,
    1
  )]);
  const tail = Buffer.from('{"nonce":"exp-0025');
  const bytes = Buffer.concat([Buffer.from(first), tail]);
  const inspected = inspectExp0025Journal(bytes);
  assert.equal(
    inspected.status,
  ...
  const tailOnly = inspectExp0025Journal(new Uint8Array([0xff, 0xfe, 0x7b]));
  assert.equal(
    tailOnly.status,
    EXP0025_JOURNAL_INSPECTION_STATES.truncatedTail
  );
...
test("frame completo malformado nunca vira tail recuperável", () => {
  ...
  const opening = serializeExp0025Journal([frame(
    EXP0025_JOURNAL_FRAME_TYPES.inProgress,
    1
  )]);
  ...
  const invalidWithTail = inspectExp0025Journal(mixed);
  assert.equal(
...
test("rejeita nonce, gap ordinal, primeiro tipo e outcome não terminal", () => {
  const opening = frame(EXP0025_JOURNAL_FRAME_TYPES.inProgress, 1);
  const worker = frame(EXP0025_JOURNAL_FRAME_TYPES.workerStarted, 2);
  const outcome = frame(EXP0025_JOURNAL_FRAME_TYPES.workerOutcome, 3);

  const wrongNonce = { ...opening, nonce: "outro" };
  assert.equal(validateExp0025JournalFrame(wrongNonce).valid, false);
  assert.equal(validateExp0025JournalFrame(wrongNonce, {
    nonce: "outro"
  }).valid, false);
  assert.throws(() => serializeExp0025JournalFrame(wrongNonce), /nonce/u);
  ...
  const workerFirst = frame(EXP0025_JOURNAL_FRAME_TYPES.workerStarted, 1);
  assert.throws(() => serializeExp0025Journal([workerFirst]), /IN_PROGRESS/u);
  assert.throws(
    () => serializeExp0025Journal([opening, worker, outcome,
      frame(EXP0025_JOURNAL_FRAME_TYPES.healthAfter, 4)]),
    /WORKER_OUTCOME/u
  );
});
...
test("rejeita recursivamente bytes, base64 e nomes de chave proibidos", () => {
  ...
  for (const trial of forbiddenValues) {
    const physicalPayload = payload(
      EXP0025_JOURNAL_FRAME_TYPES.physicalTrialResult
    );
    physicalPayload.trial = trial;
    assert.throws(() => createExp0025JournalFrame({
      ordinal: 1,
      type: EXP0025_JOURNAL_FRAME_TYPES.physicalTrialResult,
...
test("rejeita chaves extras em todos os payloads tipados", () => {
  for (const [index, type] of
    Object.values(EXP0025_JOURNAL_FRAME_TYPES).entries()) {
...
test("ledger aceita URL blob bruta sem aplicar política de origem", () => {
  const request = payload(EXP0025_JOURNAL_FRAME_TYPES.networkRequest);
  request.method = "GET";
  request.postData = null;
  request.resourceType = "Media";
  request.url = "blob:http://localhost:4173/6e93026e-a3cc-4baf-b975-f8ae125c8242";
  assert.doesNotThrow(() => createExp0025JournalFrame({
    ordinal: 1,
    type: EXP0025_JOURNAL_FRAME_TYPES.networkRequest,
    payload: request
  }));
});

test("capture-completed é limitado e nunca carrega corpo", () => {
  const valid = frame(EXP0025_JOURNAL_FRAME_TYPES.captureCompleted, 1);
  assert.equal(validateExp0025JournalFrame(valid).valid, true);
  ...
  for (const mutation of [
    ...
    assert.equal(validateExp0025JournalFrame(changed).valid, false);
  }

  const failed = createExp0025JournalFrame({
    ordinal: 1,
    type: EXP0025_JOURNAL_FRAME_TYPES.captureCompleted,
    payload: {
      ...payload(EXP0025_JOURNAL_FRAME_TYPES.captureCompleted),
  ...
  assert.equal(validateExp0025JournalFrame(failed).valid, true);

  const physicalWithoutBinding = createExp0025JournalFrame({
    ordinal: 1,
    type: EXP0025_JOURNAL_FRAME_TYPES.physicalTrialResult,
    payload: {
      ...payload(EXP0025_JOURNAL_FRAME_TYPES.physicalTrialResult),
      requestId: null
    }
  });
  assert.equal(
    validateExp0025JournalFrame(physicalWithoutBinding).valid,
    true
  );
  ...
  assert.equal(
    validateExp0025JournalFrame(qualifiedWithoutBinding).valid,
    false
  );
});

test("worker outcome fecha schema de processo e stderr", () => {
  const valid = frame(EXP0025_JOURNAL_FRAME_TYPES.workerOutcome, 1);
  assert.equal(validateExp0025JournalFrame(valid).valid, true);
  for (const outcome of [
    ...
    assert.equal(validateExp0025JournalFrame({
      ...valid,
...
test("JSON completo não canônico, duplicado ou CRLF é inválido", () => {
  ...
  assert.equal(
    inspectExp0025Journal(pretty).status,
    EXP0025_JOURNAL_INSPECTION_STATES.invalidCompleteFrame
  );
  const canonical = serializeExp0025Journal([opening]);
  const crlf = Buffer.from(canonical.replace(/\n$/u, "\r\n"));
  assert.equal(
    inspectExp0025Journal(crlf).status,
    EXP0025_JOURNAL_INSPECTION_STATES.invalidCompleteFrame
  );
  ...
  assert.equal(
    inspectExp0025Journal(duplicate).status,
    EXP0025_JOURNAL_INSPECTION_STATES.invalidCompleteFrame
  );
});

test("validação é fail-closed para ciclos e tipos não JSON", () => {
  const cycle = payload(EXP0025_JOURNAL_FRAME_TYPES.budgetInputs);
  cycle.inputs.self = cycle.inputs;
  const candidate = {
    schemaVersion: EXP0025_JOURNAL_SCHEMA,
    nonce: EXP0025_JOURNAL_NONCE,
    ordinal: 1,
    type: EXP0025_JOURNAL_FRAME_TYPES.budgetInputs,
    payload: cycle
  };
  const validation = validateExp0025JournalFrame(candidate);
  assert.equal(validation.valid, false);
  assert.match(validation.errors.join(" "), /circular/u);
  assert.throws(() => inspectExp0025Journal("not-bytes"), /Buffer|Uint8Array/u);
});
