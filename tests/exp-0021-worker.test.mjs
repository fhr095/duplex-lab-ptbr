...
import { EXP0021_NETWORK_ENABLE_OPTIONS } from
...
import {
  ...
  EXP0021_WORKER_ORDER,
  EXP0021_WORKER_PAYLOADS,
  createExp0021NetworkTracker,
  exp0021BrowserAuditSource,
  exp0021BrowserFetchExpression,
  exp0021BrowserHealthExpression,
  exp0021BoundWebSocketUrl,
  exp0021CdpUrlFromRoute,
  isExp0021AllowedNetworkUrl,
  parseExp0021TtsPostData,
  runExp0021WorkerCampaign
} from "../scripts/run-exp-0021-worker.mjs";
...
function emitUnit(tracker, input = {}) {
  ...
  const text = input.text ?? EXP0021_WORKER_PAYLOADS.A.text;
  tracker.handleEvent({
...
test("worker congela dois payloads distinguíveis e ordem A1/B1/B2/A2", () => {
  assert.deepEqual(EXP0021_WORKER_ORDER.map((unit) => unit.trialId), [
    "A1", "B1", "B2", "A2"
  ]);
  assert.deepEqual(EXP0021_WORKER_ORDER.map((unit) => unit.sequence), [
    1, 2, 3, 4
  ]);
  assert.notEqual(
    EXP0021_WORKER_PAYLOADS.A.text,
    EXP0021_WORKER_PAYLOADS.B.text
  );
  assert.equal(EXP0021_WORKER_PAYLOADS.A.rate, 1);
  assert.equal(EXP0021_WORKER_PAYLOADS.B.rate, 1);
  assert.deepEqual(EXP0021_CONFIG.networkEnable,
    EXP0021_NETWORK_ENABLE_OPTIONS);
  assert.deepEqual(EXP0021_PATHS, {
    report: EXP0021_BOUNDARY_PATHS.report,
    freeze: EXP0021_BOUNDARY_PATHS.freeze,
    attempt: EXP0021_BOUNDARY_PATHS.attempt,
    receipt: EXP0021_BOUNDARY_PATHS.receipt,
    preregistration: EXP0021_BOUNDARY_PATHS.preregistration
  });
  assert.deepEqual(
    EXP0021_TRIAL_ORDER.map(({ navigation, position, ...identity }, index) => ({
...
test("browser faz fetch e digest sem chamar fala ou playback", () => {
  const expression = exp0021BrowserFetchExpression(
    EXP0021_WORKER_PAYLOADS.A.text
  );
  ...
  const audit = exp0021BrowserAuditSource();
  for (const token of [
    "AudioContext",
    "webkitAudioContext",
    "HTMLMediaElement",
  ...
  const healthBinding = exp0021BrowserHealthExpression();
  assert.match(healthBinding, /fetch\("\/api\/health"/u);
...
test("autoridade CDP fica no gateway WSL e WebSocket no target criado", () => {
  ...
  const bound = exp0021BoundWebSocketUrl(
    "http://172.20.32.1:9223/",
    {
      id: "target-1",
...
test("tracker liga um único requestId e remove bytes/base64 do envelope", async () => {
  ...
  tracker.beginUnit({
    ...
    payloadId: "A",
    sequence: 1,
    text: EXP0021_WORKER_PAYLOADS.A.text
  });
  emitUnit(tracker);
...
test("tracker sinaliza requestId stale/cross-trial em vez de aceitá-lo", async () => {
  ...
  tracker.beginUnit({
    ...
    payloadId: "B",
    sequence: 2,
    text: EXP0021_WORKER_PAYLOADS.B.text
  });
  emitUnit(tracker, {
    requestId: "reused",
    text: EXP0021_WORKER_PAYLOADS.B.text
...
test("status/MIME/overflow falham tipados antes de ler o body", async () => {
  for (const [overrides, expectedCode] of [
    ...
    [{
      encodedDataLength: EXP0021_NETWORK_ENABLE_OPTIONS.maxResourceBufferSize
...
test("URL e postData falham fechados fora da origem/payload exatos", () => {
  assert.equal(isExp0021AllowedNetworkUrl(EXP0021_TARGET_URL), true);
  assert.equal(isExp0021AllowedNetworkUrl(EXP0021_TTS_URL), true);
  assert.equal(isExp0021AllowedNetworkUrl("https://example.com/a.js"), false);
  assert.deepEqual(parseExp0021TtsPostData(JSON.stringify({ text: "A" })), {
    valid: true,
    text: "A"
  });
  assert.equal(parseExp0021TtsPostData("{}").valid, false);
  assert.equal(parseExp0021TtsPostData("not-json").valid, false);
});
...
function fakeChrome(captureMode = "success") {
  ...
  const chrome = {
    ...
    evaluate: async (expression) => {
      if (expression.includes("EXP0021_WAIT_READY")) return true;
      if (expression.includes("EXP0021_BROWSER_HEALTH_BINDING")) {
        const requestId = `health-${requestSequence + 1}`;
        listener({
          method: "Network.requestWillBeSent",
          params: {
            requestId,
            ...
            type: "Fetch",
            request: {
              url: new URL("/api/health", EXP0021_TARGET_URL).href,
              method: "GET"
            }
          }
        });
        ...
        listener({
          method: "Network.loadingFinished",
          params: { requestId, encodedDataLength: 100 }
        });
        return {
          url: new URL("/api/health", EXP0021_TARGET_URL).href,
          status: 200,
      ...
      if (expression.includes("EXP0021_READ_NEGATIVE_BUDGET")) {
        return {
          audit: {
...
test("campanha fake executa 2x2, buffers exatos e budget negativo", async () => {
  ...
  assert.equal(envelope.schemaVersion, EXP0021_WORKER_ENVELOPE_SCHEMA);
  ...
  assert.deepEqual(envelope.campaign.budget, EXP0021_CONFIG.negativeBudget);
  ...
  assert.ok(fixture.chrome.commands.some(({ method, params }) =>
    method === "Network.enable" &&
      assert.deepEqual(params, EXP0021_NETWORK_ENABLE_OPTIONS) === undefined
  ));
