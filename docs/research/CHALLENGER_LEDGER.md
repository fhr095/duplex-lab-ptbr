...
única referência externa ainda aberta: passou 4/4 sentinelas sob a semântica
do servidor oficial, mas o adaptador fail-closed encerrou antes de `D`. Assim,
o protocolo está validado e a transferência pt-BR continua desconhecida. Uma
passagem `D`-only aguarda decisão explícita sobre ampliar o teto cumulativo de
download; ASR continua desconhecido e deferido.

## Fila atual

| Challenger / mecanismo | Evidência disponível | Decisão afetada | Menor teste e dependências | Gate | Stop rule | Status |
| --- | --- | --- | --- | --- | --- | --- |
| **DuplexCascade — tomada de turno por microturnos** | O probe local `L` fez 9→4 tomadas prematuras, 5 correções e 0 introduções em H, mas falhou p95 (1.200 ms > 800 ms) e foi equivalente a `A0@600`; `L` está cortado. O checkpoint oficial passou 4/4 sentinelas sob o runtime publicado; `D` tem zero observações. | Se `E` possui vantagem comportamental residual sob a mesma cadência que justifique registrar um gap externo não reproduzido. | Com autorização, usar uma única quarta alocação para reidratar o mesmo checkpoint sob teto cumulativo de 70 GiB e fazer uma passagem `D`-only; sem repetir sentinelas, usar H, abrir `L2`, ASR ou TTS. | O gate residual por fala/sessão e latência contra `A0@600`; falha de transferência pt-BR permanece separada de falha de protocolo. | Sem extensão, manter `D=NOT_EVALUATED`. Se `E` não vencer, cortar; se vencer, apenas justificar um H fresco sob novo pré-registro, sem trocar runtime ou backbone. | **test** |
